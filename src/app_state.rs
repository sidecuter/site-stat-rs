...
static METHODS: LazyLock<HashSet<String>> =
    LazyLock::new(|| METHODS_ARRAY.iter().map(|&v| v.to_string()).collect());
...
impl AppState {
    pub fn new() -> Self {
        ...
        let files_path = std::path::Path::new(&static_path)
            .join("images")
            .to_str()
            .unwrap()
            .to_string();
        let front_path = std::path::Path::new(&static_path)
            .join("web")
            .to_str()
            .unwrap()
            .to_string();
        ...
        let data_refresh_interval =
            std::env::var("REFRESH_INTERVAL")
                .ok()
                .map_or(Duration::from_secs(600), |v| {
                    Duration::from_secs(
                        v.parse::<u64>()
                            .expect("Refresh interval should be integer"),
                    )
                });
        Self {
            host,
