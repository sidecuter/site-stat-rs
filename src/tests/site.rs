...
use crate::api::site::add::add_stat_site;
...
async fn stat_site_endpoint(
    ...
    let app = test::init_service(App::new().app_data(Data::new(db)).service(add_stat_site)).await;
...
#[rstest]
#[tokio::test]
async fn test_400_stat_site_endpoint(
    #[future(awt)] prepare_connection: Result<DatabaseConnection, Box<dyn std::error::Error>>,
) {
    assert!(prepare_connection.is_ok());
    let db = prepare_connection.unwrap();
