...
pub async fn api_key_middleware(
    ...
    if let Some(app_state) = req.app_data::<Data<AppState>>() {
        if req.head().headers().get("Api-Key").is_some_and(|hv| hv.as_bytes() == app_state.admin_key.as_bytes()) {
            next.call(req).await
        } else {
            let (request, _pl) = req.into_parts();
            let response = ApiError::NotAllowed("Specified Api-Key is not present in app".to_string())
                .respond_to(&request);
